...
async def get_users() -> Page[UserRead]:
    users = await crud.firestore.get_all_documents(USER_COLLECTION)
...
async def add_user(userData: UserCreate) -> UserRead:
    update_time, doc_ref = await crud.firestore.add_document(
        USER_COLLECTION,
        dict(userData),
    )
    user = dict(userData)
